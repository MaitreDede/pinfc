...
#define PN532_I2C_PIGPIO_DRIVER_NAME "pn532_i2c_pigpio"

#define LOG_CATEGORY "libnfc.driver.pn532_i2c_pigpio"
#define LOG_GROUP NFC_LOG_GROUP_DRIVER

// I2C address of the PN532 chip.
#define PN532_I2C_ADDR 0x24

typedef int16_t i2c_device;
...
pn532_i2c_scan(const nfc_context *context, nfc_connstring connstrings[], const size_t connstrings_len)
{
    ...
    // const char *i2cPort;
    // int iDevice = 0;
    bool hasFoundDevice = false;

    for (uint16_t busId = 0; busId <= 2; busId++)
    {
        const uint32_t i2c_flags = 0;
        id = i2c_open(busId, PN532_I2C_ADDR, i2c_flags);
        if (id < 0)
        {
            //Open failed
            switch (id)
            ...
            case PI_I2C_OPEN_FAILED:
                log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_WARN, "Open pigpio I2C bus %d failed: %d.", busId, id);
                break;
            default:
                log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_WARN, "Open pigpio I2C bus %d failed: unknown response code %d.", busId, id);
                break;
            }
            continue;
        }
        log_put(LOG_GROUP, LOG_CATEGORY, NFC_LOG_PRIORITY_DEBUG, "Trying to find PN532 device on I2C bus %d.", busId);

        nfc_connstring connstring;
        snprintf(connstring, sizeof(nfc_connstring), "%s", PN532_I2C_PIGPIO_DRIVER_NAME);
        ...
        pnd->driver = &pn532_i2c_pigs_driver;
...
pn532_i2c_open(const nfc_context *context, const nfc_connstring connstring)
{
    ...
    int connstring_decode_level = connstring_decode(connstring, PN532_I2C_PIGPIO_DRIVER_NAME, NULL, &i2c_devname, NULL);
    ...
    snprintf(pnd->name, sizeof(pnd->name), "%s:%s", PN532_I2C_PIGPIO_DRIVER_NAME, i2c_devname);
...
const struct nfc_driver pn532_i2c_pigs_driver = {
    .name = PN532_I2C_PIGPIO_DRIVER_NAME,
    .scan_type = INTRUSIVE,
    .scan = pn532_i2c_scan,
    .open = pn532_i2c_open,
